...
double f(double x ,double y){//la forma que resuelve este metodo es ecuaciones de la forma: dy/dx = f(x,y) y necesitamos y(x0)= y0
	...
	int main(int argc, char *argv[]) {
		...
		for(int i = 1 ; i < n ; i++){ //menor a n porq cuando llegue a n-1, el for calcula el n+1
			x[i] =  x[i-1]+ h;
			if(i==1){
				y[i] = y[i-1] + h*f(x[i-1],y[i-1]);
			}
			y[i] = y[i-1] + h*(2*f(x[i-1],y[i-1])- f(x[i-2],y[i-2]));
			//if(i==19||i==39||i==59||i==79||i==99)//AGREGAR SI QUIERO QUE ME MUESTRE SOLO LOS X EQUIESPACIADOS con un h=0.2
